# -*- coding: utf-8 -*-
"""
Created on Fri Jun 14 20:55:06 2019
...
class NASNetALarge(nn.Module):
    ...
    def __init__(self, num_classes=1001, stem_filters=96, penultimate_filters=4032, filters_multiplier=2):
        ...
        self.con1_1 = nn.Conv2d(24*filters, 1, 1, 1, 0, bias = False)
...
#        self.eps = 1e-8
#
#    def forward(self, xx, yy, gtgt, batch_size):
#        
#        Loss = 0
#        for i in range(batch_size):
#            x, y, gt = xx[i], yy[i], gtgt[i]
#            vx = x - torch.mean(x)
#            vy = y - torch.mean(y)
...
#            #y_map_norm = (gt - torch.mean(gt))/(torch.std(gt) + self.eps)
#    #        print('y_map_norm',torch.mean(x),torch.max(x),torch.std(x),'x_map_norm',torch.mean(gt),torch.max(gt),torch.std(gt))
#            #diff = y_map_norm - x_map_norm
#            m = torch.sum(torch.mul(x_map_norm, gt))
#    #        print('gt',gt[0][0].topk(50, dim=1),'diff',torch.max(diff))
#    #        print('------------------________________________m______________________------------',m)
#            #print(m)
#            num = torch.sum(gt)+ self.eps#求1的个数
#            NSSloss =torch.div(m, num)
#    #        print('num',num,'NSSloss',NSSloss)
#    
#    
#            #KLD损失函数
#            max_x = torch.max(x)
...
#            
#            
#            Loss = Loss-CCloss-2*NSSloss+10*KLDloss
#        
#        
...
class Mul_loss(nn.Module):
    ...
    #        print('num',num,'NSSloss',NSSloss)
            ...
            Loss = Loss + 1-CCloss+NSSloss+KLDloss
        
        
        return Loss/batch_size
    
if __name__ == "__main__":
    model_name = "nasnetalarge-a1897284.pth"
    model = NASNetALarge()
